...
local tlast = require "Sources.tlast"
local tlst = require "Sources.tlst"
local tltype = require "Sources.tltype"
local tlparser = require "Sources.tlparser"
local tldparser = require "Sources.tldparser"
local tlfilter = require "Sources.tlfilter"
    ...
    if not var.assigned and not tlst.IsUpValue(env, var) then
      ...
      if not tltype.isProj(t) then
        ...
        if not tltype.isVoid(tf) then
          tlst.BackupVarType(env, var, pos)
          set_type(var, tf)
        end
      else
        ...
        if tltype.isUnionlist(proj) then
          ...
          if not tltype.isVoid(nproj) then
            tlst.BackupVarType(env, vproj, pos)
...
local function check_while (env, stm)
  ...
  if apply_filters(env, true, fs, exp1.pos) then -- while block is unreacheable
    ...
    local frame = tlst.PopBackup(env)
    ...
    local snapshots = tlst.PopBreak(env)
    if not r then
      snapshots[#snapshots+1] = frame
    end
    snapshots[#snapshots+1] = {}
    local newtypes = tlst.JoinSnapshots(snapshots, stm.pos)
    for var, ty in pairs(newtypes) do
      tlst.CommitType(env, var, ty, stm.pos)
    end
...
local function check_if (env, stm)
  ...
  local newtypes = tlst.JoinSnapshots(snapshots, stm[1].pos)
  for var, tyub in pairs(newtypes) do
    tlst.CommitType(env, var, tyub, stm[1].pos)
  end
  local r = true
...
local function check_forin (env, idlist, explist, block)
  ...
  snapshots[#snapshots+1] = {} -- can run 0 times
  local newtypes = tlst.JoinSnapshots(snapshots, block.pos)
  for var, ty in pairs(newtypes) do
    tlst.CommitType(env, var, ty, block.pos)
  end
  return r, didgoto
end
    ...
    if not l.assigned then
      ...
      if tltype.subtype(tr, t) then
        if tltype.isUnion(t) and (not tltype.subtype(t, tr)) then
          t = tltype.general(tr)
          tlst.CommitType(env, l, { type = t }, var.pos)
        end
      else
...
function tlchecker.typecheck (ast, subject, filename, strict, integer, color)
  ...
  local env = tlst.NewEnv(subject, filename, strict, color)
