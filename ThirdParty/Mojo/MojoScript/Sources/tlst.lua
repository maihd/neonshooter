...
function tlst.NewEnv(subject, filename, strict, color)
  local env = {}
  env.subject = subject
...
function tlst.NewProjection(env, t)
  local label = {}
  env["projections"][label] = t
  return label
end

function tlst.GetProjection(env, label)
  return env["projections"][label]
end

function tlst.SetProjection(env, label, t)
  env["projections"][label] = t
end
...
function tlst.RevertTypes(env, frame)
  local curr = {}
...
function tlst.BreakProjection(env, var)
...
function tlst.JoinSnapshots(snaps, pos)
...
function tlst.CommitType(env, var, ty, pos)
  assert(pos, "position must not be nil")
  local inframe = tlst.BackupVarType(env, var, pos)
  var.type = ty.type
...
function tlst.PushBreakSnapshot(env)
  local bkps = env["backups"]
  local brks = env["breaks"]
