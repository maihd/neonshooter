...
public:
	...
	void Set(const float2* points, int count);
	...
	void SetAsBox(float hx, float hy, const float2& center, float angle);
	...
	float2 m_centroid;
	float2 m_vertices[b2_maxPolygonVertices];
	float2 m_normals[b2_maxPolygonVertices];
	int m_count;
};
