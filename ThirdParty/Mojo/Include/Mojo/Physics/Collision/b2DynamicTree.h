...
#include "Mojo/Physics/Collision/b2Collision.h"
#include "Mojo/Physics/Common/b2GrowableStack.h"
...
public:
	...
	bool MoveProxy(int proxyId, const b2AABB& aabb1, const float2& displacement);
...
inline void b2DynamicTree::RayCast(T* callback, const b2RayCastInput& input) const
{
	...
	float2 v = b2Cross(1.0f, r);
	float2 abs_v = b2Abs(v);
	...
	while (stack.GetCount() > 0)
	{
		...
		// Separating axis for segment (Gino, p80).
		// |dot(v, p1 - c)| > dot(|v|, h)
		float2 c = node->aabb.GetCenter();
		float2 h = node->aabb.GetExtents();
